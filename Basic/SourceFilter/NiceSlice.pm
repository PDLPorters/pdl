...
BEGIN {
# fix for problem identified by Ingo, also EOP fix that needs propagating back
my $ncws = qr/\s+/;
my $comment = qr/(?<![\$\@%])#.*/;
...
my %extractor_for = (
    code_no_comments
               => [ { DONT_MATCH => $comment },
                    ...
                    $id, { DONT_MATCH => \&Text::Balanced::extract_quotelike }   ],
);
use Filter::Simple ();
...
sub my_gen_std_filter_for {
    ...
    return sub {
        my $instr;
        my @components;
        for (Text::Balanced::extract_multiple($_,$extractor_for{$type})) {
