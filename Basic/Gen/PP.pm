...
# "Other pars", the parameters which are usually not pdls.
 ...
 [[InplaceCode],        [_Inplace],           "InplaceCode",
  'Insert code (just before HdrCode) to ensure the routine can be done inplace'],
 [[HdrCode],            [],           sub { return '' },
...
#
# If $bvalflag (ie $PDL::Config{WITH_BADVAL}), then we have to worry
# about checking for the badvalue/data/whatever you want to call it flag
# in each piddles state *\(unless $badhdrcode is supplied, when that's
# assumed to do it)
# 
sub VarArgsXSHdr {
...
# - this code flags the routine as working inplace, where
# the argument to the Inlace rule is a two element array,
# the first element is the name of the input piddle
# and the second argument the name of the output piddle
# ie "Inplace => [ 'a', 'b' ]"
#
sub InplaceCode {
    my ( $aref ) = @_;
    return '' unless defined $aref;
    die "ERROR: Inplace must be sent a ref to a 2 element array\n"
	unless $#$aref == 1;
    my $a = $aref->[0];
    my $as = $a . "->state";
    my $b = $aref->[1];
    ...
    return 
	"\tif ( $as & PDL_INPLACE ) {
              $as &= ~PDL_INPLACE; /* unset */
              $b = $a;             /* discard output value, leak ? */
              PDL->SetSV_PDL(${b}_SV,${b});
          }",
