...
Current FAQ version:  0.91


=head1 DESCRIPTION

This is version  0.91 of the PDL FAQ, a collection of  frequently 
...
requires version 5.6.2 of Perl, version 5.8 or higher is
...
from there.
 ...
 [ Need to update with information for win32 ]
...
As of this writing (FAQ version 0.91 of 03 Oct 2009) the latest version is 2.4.4. 
The latest stable version should always be available from a CPAN mirror site near
you (see L<Question 2.7|"Q: 2.7    Where do I get it?"> for info on where to get PDL.

The most current (possibly unstable) version of PDL can be obtained 
from the Git repository, see L<Question 4.10|"Q: 4.10    How do I get PDL via Git?">
...
or through the Bugs link on http://pdl.perl.org .  See the BUGS
...
Next, check the file perldl.conf to see if by editing the
...
for future builds just copy it as ~/.perldl.conf into your home directory
...
distibution's package manager as many now include PDL.  PPMs
...
tested--input is welcome.  There is also a distribution of PDL
through ActiveState's ppm, though it might not always be the latest version.
...
L<Git|http://git-scm.com> source control management system.  Note that
some auxiliary project repositories (the project website, some books,
etc.) are still available via CVS.


=head2 Q: 4.10    How do I get PDL via Git?
...
To get developer read/write access to the repository type at the
command line

   git clone ssh://USERNAME@pdl.git.sourceforge.net/gitroot/pdl/pdl PDL


=head2 Q: 4.11   I had a problem with the Git version, how do I check if  someone has submitted a patch?  
...
=head2 Q: 4.12   I have gotten developer access to Git, how do I upload my changes?
...
PDL's SF Git page http://sourceforge.net/scm/?type=git&group_id=612
...
Then update the shared repository (at SF) with your changes
...
using lvalue subroutines, see L<PDL::Lvalue> for more info.  
...
released version of PDL 2.4.3 [ What about 2.4.4? ].
...
There are also C<3dtest.pl> and C<line3d.pl> in the main PDL
...
use of perl 5.6.2 (a.k.a. perl 5.006002) is now required    
