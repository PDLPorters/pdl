...
Current FAQ version:  0.8


=head1 DESCRIPTION

This is version  0.8 of the PDL FAQ, a collection of  frequently 
...
mailing list at perldl@jach.hawaii.edu. See below for instructions 
...
(see below), a code generator/parser/pre-processor that generates
...
processing. This will probably be achieved by having 
L<PDL::PP|PDL::PP> (see below) generate appropriate code on 
...
requires version 5.6 of Perl, version 5.8 or higher is
I< strongly> recommended. More information on where and how 
to get a Perl installation can be found at the Perl home page 
http://www.perl.org and at many CPAN sites (if you do
not know what I< CPAN> is check the answer to the next question).
...
might be a binary distribution availabe, see "Binary distributions"
below.
...
is http://www.cpan.org , and local CPAN sites (mirrors) can be
found there. Within CPAN you find the latest released version of
PDL in the directory /modules/by-module/PDL/. PDLs homepage is at
http://pdl.perl.org and the latest version can also be downloaded
from there.
...
C< man PDL> get you started, C< man PDL::FAQ> will let you read this 
document, etc. In any case (i.e. also on non-unixes) 
C< perldoc PDL::Intro> should work, and if you don't want to type that
much, C< pdldoc intro> should also work.
...
http://www.xray.mpe.mpg.de/mailing-lists/pdl-porters/ . More recent messages 
...
As of this writing (FAQ version 0.8 of July 2008) version 2.4.4 is
nearing release. The latest stable version should always be available
from a CPAN mirror site near you (see L<Question 2.7|"Q: 2.7    Where do I get it?">
for info on where to get PDL.

The most current (possibly unstable) version of PDL can be obtained 
from the CVS repository, see L<Question 4.8|"Q: 4.8    Can I get PDL via CVS?">
...
mailing list at pdl-porters@jach.hawaii.edu . See the subscription instructions in 
L<Question 3.2|"Q: 3.2    Are there other PDL information sources on the internet?">.
...
searchable archive of the PDL mailing list to find whether
this bug has already been discussed. If you still haven't found
any explanations you can post a bug report to 
perldl@jach.hawaii.edu , or through the Bugs link on http://pdl.perl.org .
...
upgrade to a version (>2.005) which should have this
fixed. 
...
=head2 Q: 4.5    How can I install PDL in a non-standard location?
...
distibution's package manager as many now include PDL.


=head2 Q: 4.7    Does PDL run on Linux? (And what about packages?)  
...
=head2 Q: 4.8    Does PDL run under Windows?  
  

PDL builds fine on Win32 using MinGW or Microsoft compilers.  See
the Win32/INSTALL file for details. Other compilers have not been
...
=head2 Q: 4.9    Can I get PDL via CVS?   
  

Yes, as of December 1999, PDL is available at the CVS repository on
http://www.sourceforge.net . The tree is updated by developers who
have accounts on Sourceforge and snapshots of the tree are released
regularly by the pumpkin holder (the pumpking).

If you wish to access the CVS repository and install PDL from there
all you need are two simple commands, however make sure you read some
of the documentation on Sourceforge as well for full information, but
the basic command is case sensitive and is:
...
that the CVS tree is to be considered a development release and as
such you are very welcome to try it out, but it is not recommended for
mission critical use and might crash unexpectedly.




=head2 Q: 4.10   I had a problem with the CVS version, how do I check if  someone has submitted a patch?  
...
=head2 Q: 4.11   I have gotten developer access to CVS, but I have trouble  committing uploads.  
...
When mentioned in the INSTALL directions and possibly
...
of perl objects, i.e. blessed thingies (see C<man perlobj> )) are
...
C< at> /
C< set> (see
L<the section 'Sections' in PDL::Impatient|PDL::Impatient/"Sections"> ) or the powerful 
C< slice> function and friends (see 
L<PDL::Slices|PDL::Slices> and 
L<PDL::Indexing|PDL::Indexing> ).

Finally, to confuse you completely, you can have perl arrays
of pdls, e.g. $spec[3] can refer to a pdl representing ,e.g,
...
documentation (see L<PDL::Impatient>):
...
subroutines, and this is now incorporated into PDL by 
requiring at least Perl 5.6.
...
basis. So it is really very simple: Do not use logial operators on 
...
L<PDL::Objects|PDL::Objects> (e.g. type 
...
t/subclass.t in the PDL distribution.
...
C< rcols> and C< rgrep> functions, also in PDL::IO::Misc.
...
rpic (see L<PDL::IO::Pic|PDL::IO::Pic> ) you could say:
...
where C< < testfile >> ; should match the pattern 
C< test[3-9].p> and watch the results. Some of the tests should bring up a window
where you can control (twiddle) the 3D objects with the mouse. Try using
MB1 for turning the objects in 3D space, MB3 to zoom in and out, and
...
entries in the drivers.list file. Then when running 'make' you
...
The first stop is again C< perldl> or the PDL documentation. There is 
already a lot of functionality in PDL which you might be aware of. 
The easiest way to look for functionality is to use the C< apropos> command:
...
the L<PDL::PP|PDL::PP> manpage and
...
The L<PDL::PP|PDL::PP> manpage is the main
...
General minor update: mailing list info, links, version, date, some
text.
...
form. Ask on the PDL mailing list
