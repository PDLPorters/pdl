...
This is version 0.6 of the PDL FAQ, a collection of
frequently asked questions about PDL - the Perl Data Language.

This FAQ was generated on 5.25.00.

Current maintainer: Christian Soeller (F<c.soeller@auckland.ac.nz>).

=head2 Where to find this document

You can find the latest version of this document at F<http://pdl.perl.org/Doc/pdlfaq.html>.
...
FAQ maintainer Christian Soeller (F<c.soeller@auckland.ac.nz>). See below for instructions on how to join
the mailing lists.
...
As of this writing (FAQ version 0.6 of 5.30.00) the
latest released version is 2.005. However, you should always check
CPAN, or the PDL web page F<http://pdl.perl.org> to see if there is a later
released version. (see above for info on where
to get PDL).
...
On CPAN you can find the PDL-NetCDF module. This provides a object-
oriented interface from PDL to netCDF-format data-files.
...
C<null> is a special type of 'potential piddle' or 'target piddle'. A
null pdl can be used to flag to a PDL function that it should create
an appropriately sized and typed piddle. I<Null> piddles can be used
in places where a PDL function exspects an I<output> or I<temporary>
argument. I<Output> and I<temporary> arguments are flagged in the
I<signature> of a PDL function with the C<[o]> and C<[t]> qualifiers
(see next question if you don't know what the I<signature> of a PDL
function is).  For example, you can invoke the C<sumover> function as
follows:
...
Note that this concept of null piddles is quite different from the
concept of C<empty> piddles, which are piddles that have zero elements.
...
For other examples check the scripts F<t/subclass.t>, F<t/subclass2.t>, etc. in the PDL
distribution.
...
current FAQ maintainer Christian Soeller (F<c.soeller@auckland.ac.nz>).
...
(F<c.soeller@auckland.ac.nz>)) to compile a list of the most
...
Christian Soeller (F<c.soeller@auckland.ac.nz>) or ask on the PDL mailing list F<perldl@jach.hawaii.edu>
if you are unclear.
