...
PDL::Migration::MATLAB - A guide for MATLAB users.
...
One popular IDE for Perl is caleld Padre (L<http://padre.perlide.org>).
...
objects explicitly with the C<print> command or the shotcut C<p>:
...
PerlDL:
 ...
 perldl> $a = 12    # No output.
 perldl> print $a   # Print object.
 12
 perldl> p $a       # "p" is a shorthand for "print" in the shell.
 12
 perldl>
...
=item Linearly spaced sequences
  ...
  PerlDL:   perldl> p zeroes(5)->xlinvals(2,10)
            [2 4 6 8 10]
...
method is more flexible:
  ...
  perldl> p zeros(5,5)->xlinvals(2,10)
  [
   [ 2  4  6  8 10]
   [ 2  4  6  8 10]
   [ 2  4  6  8 10]
  ...
  perldl> p zeros(5,5)->ylinvals(2,10)
  [
   [ 2  2  2  2  2]
   [ 4  4  4  4  4]
   [ 6  6  6  6  6]
  ...
  perldl> p zeros(3,3,3)->zlinvals(2,6)
  [
   [
    [2 2 2]
    [2 2 2]
...
2) In MATLAB you think "rows and columns". In PDL, think "x and y".
  ...
  -------------------------------------------------------
  (row = 2, col = 1)             (x = 0, y = 1)
  >> A(2,1)                      perldl> p $A(0,1)
  ans =                          [
         4                        [4]
                                 ]
  -------------------------------------------------------
  (row = 2 to 3, col = 1 to 2)   (x = 0 to 1, y = 1 to 2)
  >> A(2:3,1:2)                  perldl> p $A(0:1,1:2)
  ans =                          [
         4   5                    [4 5]
         7   8                    [7 8]
                                 ]
...
works over the entire piddle.
  ...
  perldl> p maximum $A
  [5 4]
  perldl> p maximum transpose $A
  [4 5 4]
  perldl> p max $A
  5
  perldl> p max ones 2,2,2
  1
  perldl> p max ones 2,2,2,2
  1
...
=item MATLAB sees a vector as a 2D matrix.
  ...
  MATLAB                       PerlDL
  ------                       ------
  >> vector = [1,2,3,4];       perldl> $vector = pdl [1,2,3,4]
  >> size(vector)              perldl> p dims $vector
  ans = 1 4                    4
...
=item But MATLAB ignores the last dimension of a 4x1x1 matrix.
  ...
  MATLAB                       PerlDL
  ------                       ------
  >> A = ones(4,1,1);          perldl> $A = ones 4,1,1
  >> size(A)                   perldl> p dims $A
  ans = 4 1                    4 1 1
...
=item And MATLAB treats a 4x1x1 matrix differently from a 1x1x4 matrix.
  ...
  MATLAB                       PerlDL
  ------                       ------
  >> A = ones(1,1,4);          perldl> $A = ones 1,1,4
  >> size(A)                   perldl> p dims $A
  ans = 1 1 4                  1 1 4

=item MATLAB has no dirext syntax for N-D arrays.

  perldl> $A = pdl [ [[1,2,3],[4,5,6]], [[2,3,4],[5,6,7]] ]
  perldl> p dims $A
  3 2 2
...
So, in the previous examples:
  ...
  # This declares two local variales and initializes them to 0.
  my ($i, $total) = (0, 0);
...
"I<structure arrays>". Per'ls arrays and hashes offer similar functionality
...
Perl hashes are similar to MATLAB's structure arrays:
  ...
  PerlDL
  ------
  perldl> %drink = ( type => 'coke' , size => 'large', mypiddle => ones(3,3,3) )
  perldl> $drink{type} = 'sprite'
  perldl> $drink{price} = 12   # Add new field to hash.
...
PDL's Pre-Processor is one of PDL's most interesting features. You write
a simple function definition and the pre-processor generates real C code
which is automatically compiled. With PDL:PP you get the full speed of
native C code without having to deal with the complexities of the C language.
...
PDL has powerful plotting abilities. Unlike MATLAB, PDL relies more on
...
Copyright 2010 Daniel Carrera <dcarrera@gmail.com>

This document is free software. You can modify and distribute it under
...
their inmense help reviewing earlier drafts of this guide. Without their
