...
All truly fast code is written in languages like FORTRAN or C and
compiled down to machine code. If you have written a function in C
or FORTRAN to perform a critical piece of analysis, you can interface it
with PDL using PDL's Pre-Processor. C<PDL::PP> is both easier to use
than Perl's standard hook mechanism (see L<perlxs|perlxs>), it will also
handle all indexing and threading for you.
