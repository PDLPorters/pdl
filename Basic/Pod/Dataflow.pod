...
This presents a problem, because the penultimate operation modifies
C<$x>, and through backwards dataflow, C<$w>. The problem is that C<$w>
is "subscribed" to updates from C<$u> and also C<$w>, so the next time
either of them get updated, that modification gets overwritten.
...
You'll notice that updating C<$u> updated C<$y>, the changes to C<$y>
that resulted from adding 50 to the diagonal (via C<$x>, and two-way flow)
got lost. This is one-way flow.
