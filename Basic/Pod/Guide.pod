...
PDL Overview - A place for me to record how PDL is
conceptually structured.

=head1 AUTHOR, DATE

This is written by David Mertens and was begun in January, 2010.  The
last update was made on April 29, 2010.

=head1 Preface

PDL is a huge project.  Some of its files address deep core
magic while others address IO and visualization and still
others address major numerical needs.  How are all of these
related?  Here is my attempt at bringing order to the chaos.

I've broken down everything by level of expertise, and within
expertise I've covered documentation, library, and workflow modules.
The documentation modules are useful for what they tell you;
the library modules are useful for the functions that they
define for you; the workflow modules are useful for the way
that they allow you to get your work done in new and different
ways.

An alternative to this document is L<PDL::Index>, which is automatically
generated from the PDL documentation index. As its name indicates, it
is not a guide. It is a catalog of all of the PDL modules on your
machine. If you are just trying to get your head around how to use PDL,
start here.
...
=item * L<PDL::Intro>, L<PDL::Impatient>, L<PDL::Philosophy>

A couple of brief introductions to PDL.  The middle one is a bit
more hands-on.  If you are new to PDL, you should start with
these.
...
PDL came of age right around the turn of the Milleneum and
...
The main workhorse module.  You'll include this in nearly
every PDL-enabled Perl script that you write.
...
This rather haphazard document contains many gems; you should read
it at some point.
...
Although writing PDL::PP code is considred an Advanced topic, and is
...
A module to help generate and work with Gaussian distributions.
...
piddles. If you find yourself writing scripts to procss many data files,
...
namespace polution and load time.
...
makefiles (but see L<PDL::pptemplate> for help on that).

=item * L<PDL::PP>

The PDL PreProcessor, which vastly simplifies making you
...
distribute your own module on CPAN, use L<PDL::Lite> or L<PDL::LiteF>
...
lvalues.  This module enables certain functions to return lvalues, like
C<slice>, which makes for cleaner code.  While good, the functionality
in this module could probably be encorporated into the core these days
since I can't imagine too many folks are running Perl 5.005 or earlier.
...
example.  Since prety much all of these operators and functions are
