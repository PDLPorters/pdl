...
The file C<Core.pm> uses some of these fields and the file C<pdlhash.c> converts
these to C when necessary.

The pdl struct is defined in C<pdl.h> and the meanings of the fields
are
...
here, the trans field in C<$c> contains a pointer to a transformation.
Only when C<$a> or C<$b> is changed, is the transformation destroyed and
...
These are required to correspond exactly to dims. 
If you want to optimize for affine transformations, use the trans or vtrans.
...
C<threadids[0]-1>, the thread dimensions with id 0 extend from
C<threadids[0]> to C<threadids[1]-1> and the thread dimensions with the last id
extend from C<threadids[nthreadids-1]> to C<threadids[nthreadids]-1>.
For example, if a pdl has dimensions C<(2,3,4,5)> (= 120 elements)
and C<nthreadids==2> and C<threadids={1,3,4}>, there is one "real" dimensions
...
transformations are generated with 
L<PDL::PP|PDL::PP> so it's better to see
that documentation.
...
The file C<pdlthread.c> handles most of the threading matters.
The threading is encapsulated in the structure C<pdlthread.h>.

=head2 Accessing children and parents of a piddle

NOTE: in the main CVS branch, only F<pdlapi.h> exists. You will
have to get the bad-value support branch to look at the other
files. However, the description below is still valid.

The file F<Basic/Core/pdlapi.h.PL> contains useful routines for
manipulating the pdl structure (it's probably easier to read
F<Basic/Core/pdlapi.h> once you've performed a build of PDL).

An example of processing the children of a piddle is provided
by the C<baddata> method of PDL::Bad (only available if you
have comiled PDL with the C<WITH_BADVAL> option set to 1,
...
well, so that I get a 'B' in the I<State> field:
...
are unimportant).
What I<is> important is that we call C<propogate_badflag> on this
...
2000 Doug Burke (burke@ifa.hawaii.edu).
