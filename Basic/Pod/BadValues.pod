...
this file contains the values of the C<WITH_BADVAL> and C<BADVAL_USENAN>
variables, and should be used by code that is executed before the F<PDL::Config>
...
When C<BADVAL_USENAN> is true, the routines in C<PDL::Ops> shoud
just fall through to the C<Code> section - ie don't use C<BadCode> -
...
=head2 What to do with boolean operations?

What should C<$a != 2> return for bad values: 0 or the bad value?

=head2 Dataflow of the badflag

Currently changes to the bad flag are propogated to the children of a piddle,
but perhaps they should also be passed on to the parents as well.
...
(ie binary incompatability) and the routines in C<Basic/Core/pdlconv.c.PL>
