...
but for the integer types special values have to be used.
...
returned - ie C<byte-E<gt>badvalue(-26)> returns 230 on my linux machine.

Note that changes to the bad value are I<NOT> propogated to previously-created
...
See discussion below.  It's not a problem for floating-point types, since
you can't change their badvalue.

=head2 A bad value for each piddle, and related issues

The following is relevant only for integer types, where there is a choice
of value to use as the bad flag.
...
floating-point types (ie those routines with C<NoBadifNaN =E<gt> 1>).
...
(hopefully) sensible values.  For floating-point numbers, we use the
C<finite()> function to check whether it is good or not.
See F<PDL/Bad/bad.pd> for read/write routines to
the values.

The default/original bad values are set to (taken from the Starlink
distribution for the integer types):
...
Note that, for the floating point types, all of C<NaN>, C<+Inf>, and
C<-Inf> are taken to be bad.  Also, these can't be changed, unlike the 
integer types.
...
C<__privtrans-E<gt>bvalflag> depending on the state of the bad flag
...
C<__privtrans-E<gt>bvalflag> is true after the code has been
...
which is done (although we may want to allow a fall back for machines
...
the computer process the bad values (ie C<NaN> and C<Inf> values) instead
of using the code in the C<BadCode> section.  Many such routines have
been labelled using C<NoBadifNaN =E<gt> 1>; however this is currently 
ignored by PDL::PP.
...
otherwise we use the C<BadCode> section.  This is going to take some
hacking of PP::PDLCode!
...
so it must be created as soon as possible in the build process.
...
I think it needs to handle bad values in the mask array.
...
make wfits write out the BLANK keyword for bad values.

=item *

get rndf/wndf to convert bad values
