...
As of PDL 2.040, you can have different bad values for separate piddles of the
same type. This currently does not work with the C<BADVAL_USENAN> option;
if you set the C<BADVAL_USENAN> value, PDL will not build.
...
C<NaN>, C<+Inf>, and C<-Inf> all to be bad. In this case:
  ...
  ISBAD   becomes   isfinite(piddle) == 0
  ISGOOD            isfinite(piddle) != 0
  SETBAD            piddle          = NaN
