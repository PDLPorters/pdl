...
C<propogate_badflag> in F<Basic/Bad/bad.pd>). 
...
Actually, I'm vacillating on this one.
...
C<<pdl nameE<gt>_badval>, which contains the badvalue for that piddle (see
...
It's currently a NOP. Other files may
still contain the option C<UseBadforNaN>, which I'd added to flag those 
routines which would need to retain the C<BadCode> segment. However, there are 
...
C<$pdl-E<gt>badvalue(23)> should change the bad value in C<$pdl> to 23
...
How about a C<setbad()> method like C<set>, eg

 $a->setbad(1,1);  # sets (1,1) bad (and ensures bad flag is set)
...
section by C<NoBadifNaN =E<gt> 1> - a previous attempt has left 
C<UseBadifNaN =E<gt> 1> dotted around the place, these need to be
removed.
