...
they should be checked for. Thanks to Christian, L<PDL::Bad/badflag> - which
sets/clears this flag - will update I<ALL> the
children/grandchildren/etc of an ndarray if its state changes.
...
doesn't mean that C<$x> shouldn't have its badflag cleared.
however, after
