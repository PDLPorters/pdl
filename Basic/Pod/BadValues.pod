...
where NaN is currently a bit of a hack 
(see Core.xs.PL for C<#include E<lt>limits.hE<gt>>).
...
sure there are problems regarding NaN stuff.  Need to add a function
to bad.pd - can_change_badvalue($type) which returns true if you
can change the badvalue used to represent this type.
