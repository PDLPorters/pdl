...
into a piddle struct at the C level. The following script
demonstrates a simplified example case:
...
and initializes the pointer to the C<Core> struct (by default the C variable
C<PDL> is used). For those who know their way around the Perl API here
is the code:
...
With these preparations your code can now access the core routines
as already shown in some of the examples above, e.g.
...
function of this struct are briefly explained.
...
to turn it into a proper piddle. Example:
  ...
  pdl *p = PDL->pdlnew();
  p->setdims(dims,ndims);
  p->set_datatype(PDL_B);
...
C<void qsort_X(PDL_Xtype *data, int a, int b)>
