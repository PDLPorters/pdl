...
C<ndims> as zero.
   ...
   static pdl* new_pdl(int datatype, PDL_Indx dims[], int ndims)
   {
     pdl *p = PDL->pdlnew();
     if (!p) return p;
     pdl_error err = PDL->setdims(p, NULL, 0);  /* set dims */
...
Affine transformation used only by C<set_datatype> to convert an ndarray's
type. Works in place if it can. Not bad-value aware.
...
ndarray's type. Unlike C<converttype>, also copies over C<threadids>. Not
bad-value aware.
...
Set the C<data> member to contain ndarray's number of elements.
