...
As of PDL 2.058, the threaded dimension sizes do not need to divide
...
pthreads will be the size of the largest threaded dimension.

In order to minimise idle CPUs on the last iteration at the end of
the threaded dimension, the algorithm that picks the dimension to
pthread on aims for the largest remainder in dividing the pthread
target into the sizes of the threaded dimensions. For example, if
a PDL has threaded dimension sizes of (9,6,2) and the I<auto_pthread_targ>
