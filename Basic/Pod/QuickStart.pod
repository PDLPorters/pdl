...
PDL::Book::QuickStart - Quick start guide to PDL
...
And the empty C<qw()>  quotes are regonised as meaning 'nothing'.
...
Here are some ways of creating a PDL variable:
 ...
 $b = pdl <<NEWPDL             # Ditto
   ...
   [4 5 6]
 NEWPDL
 $edge1 = q[1 -2];             # 2-element piddle containing  1 and -2
 $edge2 = q[1 - 2];            # 1-element piddle containing -1
...
we'll describe the most imporant one here.
...
The perl script C<pdl2> (or C<perldl>) provides a simple command line - if the latest
Readlines/ReadKey modules have beeen installed C<pdl2> detects this
