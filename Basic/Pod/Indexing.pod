...
This manpage is still in alpha
...
square brackets ([]), e.g. [ Hopefully I will be able to remove this paragraph
...
C<perldl> shell that comes with pdl.
]
...
[Note: perl v5.6.0 does allow this, but
it is an experimental feature. However, early reports suggest it works
...
greyscale. The RGB image is represented by a 3-dim pdl C<im(3,x,y)> where
...
dimensions of the autocreated pdl (here C<0D+2D=2D> to yield a 2D
...
according to R4.  
More importantly, the two L<sumover|PDL::Primitive/sumover> operations show
...
functions like L<sumover|PDL::Primitive/sumover>, 
L<prodover|PDL::Primitive/prodover>, L<minimum|PDL::Primitive/minimum> and
L<maximum|PDL::Primitive/maximum>. 
...
definition of C<sum> (as defined in C<Basic.pm>):
...
mabe dataflow as well, see L<PDL::Dataflow>). For further details
check L<PDL::PP>.
