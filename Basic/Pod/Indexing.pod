...
"threading" with ndarrays. The term threading has many different meanings in
...
The other ingredient for threading (apart from the ndarrays involved) is
a function that is threading aware (generally, these are 
L<PDL::PP> compiled functions) and that the ndarrays are "threaded" over.
...
with the threading rules). Dimensions of any ndarray argument are broken
...
If an ndarray doesn't have a loop dimension, in the thread loop this
...
dimension of this ndarray to be threaded over first leading to a thread
...
of explicit threading the dimensions of an ndarray are conceptually split into
...
thread loops (which apply to the special case where none of the ndarray
...
number of thread dimensions taken over the set of ndarray arguments.
...
maximal size found in any ndarrays having this thread dimension (for
...
If an ndarray doesn't have a I<thread/extra dimension> that corresponds to
...
All ndarrays that do have I<thread dimensions> must have the same number of
thread dimensions.
...
on implicit threading: if any of the output ndarrays has an (explicit or
