...
noted above, which get copied in. However, this can be overridden by
supplying C<Comp> and C<MakeComp> keys:
...
# advantage of not making things physical.
...
Calls C<pdl_barf> with the supplied (var-args) message, adding the
