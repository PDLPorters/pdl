...
on the assumption it will use C<#ifdef PDL_BAD_CODE> to handle bad values.
...
The C<MakeComp> code is placed in the C<pdl_(funcname)_run>, so access
to C<Pars> (which will just be C<pdl *>s)/C<OtherPars> values is just
via their names, not a macro.
...
There is an XS F<typemap> entry (only for C<PDL_Indx> array types for
now) that adds a C<(varname)_count> variable having extracted the index
...
(here, C<pdl_diagonal_run>)'s caller (here, the generated XS function)
...
These expand to a C<pdl *> pointer, e.g. to pass to a PDL API function.

=item $CHILD()

=item $PARENT()

These expand to a lookup into C<pdl> struct, e.g. C<$CHILD(ndims)>
looks up the C<ndims> value for the child parameter.
...
As well as the above-mentioned C<PDL_BAD_CODE>, there are also
C<PDL_GENTYPE_REAL>, C<PDL_GENTYPE_UNSIGNED>, and
C<PDL_GENTYPE_INTEGER>. These will be defined when the operation's
generic type has those characteristic(s):

  #ifdef PDL_GENTYPE_INTEGER
...
C<GenericTypes> to be all of them, no C<HaveThreading> or C<CallCopy>,
...
Default true. If so, generate code implementing threading (see
L<PDL::Indexing>).
...
=head3 $SETDELTATHREADIDS()

Similarly again, this sets the child's C<nthreadids> to the same as the
parent's, allocates space for the C<threadids>, then sets the child's
ones to the same as the parent's plus the given value.

To get a flavour of what C<threadids> are for, in the normal way of things
