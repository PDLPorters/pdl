...
we actually want the first part of code. This is where the C<BadCode>
...
There are also C<BadBackCode> and C<BadRedoDimsCode>, to go
with the C<BackCode> and C<RedoDimsCode> options, although they
...
Setting this key marks the routine as working inplace. Inplace should be sent
an array reference, where the first element is the name of the input piddle
and the second element the name of the output piddle (it should be clever
enough to work this out by itself!). For instance, with
...
Mark a function as being able to work inplace. The argument should be
an array reference; the first element is the name of the input piddle,
the second the output piddle (it should be made to work this out from the
...
Macros labelled by (bad) are only useable in BadXXXX sections.
