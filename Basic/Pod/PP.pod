...
including C<SV*> and C<char*>. Since C<SV*>s will be mortal copies,
code that needs them to survive will need to memory-manage them.
...
Care should be exercised in writing F<typemap> entries. For instance,
the PDL one currently has (for input):

  T_PDL_DIMLIST
          PDL_Indx ${var}_count;
          $var = PDL_CORE_(packdims)($arg, &${var}_count);

PP removes from input typemap entries everything up to the assignment,
which here removes the C<${var}_count> declaration, which is instead
added in a C<PREINIT> section thanks to PP's knowledge of this type. This
is necessary because the inputs are handled within their own scope,
since there are several branches where the inputs can be set thanks to
variable-argument handling. The count variable would be lost if such
removal were not done, since its value would be set within the local
scope, then discarded. If this causes a problem for your application,
please ask one of the PDL mailing lists or on IRC.
