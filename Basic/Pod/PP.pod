...
To prevent a single function from being exported, see L<NoExport|NoExport>.
...
want to advertise that C<bar_pp> exists. To do this, you need to set
a few of the keys in pp_def.

To ensure that no documentation (even the default PP docs) are not
generated, set

 Doc => undef,
 
To prevent the function from being added to the symbol table, set
 
 PMFunc => '',
 
To prevent the function from being added to the C<EXPORT> list, set
 
 NoExport => 1
 
These three keys will effectively make your PP function "private."
However, it is I<always> accessible via PDL::bar due to Perl's
module design. But making it private will cause the user to go very
far out of his or her way to use it, so he or she shoulders
the consequences!
...
To remove your function's name from the C<EXPORT> list, see L<NoExport|NoExport>.

B<Please note> that it is possible to use PMFunc to do silly things. For example,

  C<pp_def('foo', Code => 'printf("%s\n", "Hello, world!");', PMFunc => '*bar = \&PDL::foo;');>

will have an entry in @EXPORT called 'foo' (because PP puts it there by default), but
the symbol table will contain 'bar'. If you insist on using a different name for
your function in the symbol table than what is in the C<EXPORT> list, please also
use the L<NoExport|NoExport> key to prevent users from being confused.
...
By default, PP will add your function's name to the C<EXPORT> list. You can
prevent this by setting

 NoExport => 1
 
during a pp_def declaration.
...
programmers to use your functions as methods. To prevent a single function 
from being exported, see L<NoExport|NoExport>.
