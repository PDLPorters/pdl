...
Almost everything having to do with slice operations. This includes much
of the following (each entry is followed by a guess/description of where
...
Defined in L<PDL::PP::Rule::MakeComp>, allocates memory for any C<Comp>
arrays, after their size has been determined, e.g. here after
C<$COMP(whichdims_count)> has been set:
...
SETDIMS and SETNDIMS are used in affine
transformations (specifically, ones which set P2Child to true)
such as slice and mv.
