...
though at the moment it is not distributed with PDL.  Also, PDL::IO::FlexRaw
and PDL::IO::FastRaw read and write data identical to FORTRAN 77 UNFORMATTED
files, and FlexRaw and Storable provide general data storage capabilities.
Finally, PDL can read Grib (weather-data) files using the CPAN module
PDL::IO::Grib.
...
Provides functionality similar to L<Data::Dumper> for piddles.
L<Data::Dumper> stringifies a data structure, creating a string that
can be C<eval>ed to reproduce the original data structure.  It's also
...
Copyright (C) 2010 David Mertens, dcmertens dot perl at gmail dot com.
