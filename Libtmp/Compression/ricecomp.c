/* PDL: copied from CFITSIO 4.5.0, modified so rdecomp returns char *, and rcomp takes char ** first for error messages */
/* replace EOF with -1 */
/* remove fitsio2.h and string.h */
/* replace ffpmsg with *ret = OR return */
/* end of PDL bit */
/*
...
int fits_rcomp(char **ret, int a[],		/* input array			*/
...
*/
    ...
    if (diff == (unsigned int *) NULL) {
        *ret = "fits_rcomp: insufficient memory";
	return(-1);
    }
    /*
 ...
 *
 * Bit output routines
 * Procedures return zero on success, -1 on end-of-buffer
 *
...
char *fits_rdecomp (unsigned char *c,		/* input buffer			*/
...
*/
	...
	if (c > cend) {
            return "decompression error: hit end of compressed byte stream";
	}
    }
    if (c < cend) {
...
char *fits_rdecomp_short (unsigned char *c,		/* input buffer			*/
	     int clen,			/* length of input		*/
...
char *fits_rdecomp_byte (unsigned char *c,		/* input buffer			*/
	     int clen,			/* length of input		*/
