...
As of 2.073, you are recommended to use C<PDL_IF_BAD(iftrue,iffalse)>:
...
supplied. This can be overridden by supplying C<InplaceDoc>.
...
Hash-ref of parameter name to POD description of that parameter.
Ensure the case of the parameter name is correct, as any missing
parameters (docs are generated in order of the parameter list) will
be omitted. Intended to take care of the surrounding POD markup,
for easy, consistent-looking documentation with no effort.
Added in PDL 2.099.
...
As of 2.104, implies C<P2Child> and C<Lvalue>.
As of 2.107, implies C<HandleBad>.
...
C<iflags> such that the trans will start with forward dataflow,
and backwards if C<BackCode> or C<AffinePriv>. Note that setting
this to any value (including 0) will trigger the behaviour.
...
such that the function is a dataflowing identity transformation.

=head3 BackCode

For dataflowing functions, this value (which gets parsed) overrides the
operation of that from children ndarrays to parents.
As of 2.104, implies C<DefaultFlow> and C<Lvalue>.

=head3 BadBackCode

Same but taking account of bad values. It is now recommended you
use C<BackCode> with the C<PDL_IF_BAD> macro.
...
As of 2.104, automatically set if there is backwards dataflow, see
L</DefaultFlow>.

=head3 Synonyms

If present, must be an array-ref of additional names for the operation.
Added in 2.105.
