...
broadcasting features of I<PDL>.

Like all vectorized languages, PDL automates looping over multi-dimensional data structures ("ndarrays") using
a variant of mathematical vector notation.  The automatic looping is called
"broadcasting", similar to NumPy and Julia. PDL also automatically runs
broadcasting computation in parallel - see L<PDL::ParallelCPU>.

A lot of the flexibility and power of PDL relies on the indexing and broadcasting
...
=head2 Memory layout and indexing order

PDL follows a C-like memory layout; the fastest-moving index is
generally thought of as a column-index into a row. This is due to
PDL's roots in image-processing; when a 2D ndarray is printed out,
the 0-th dim is printed as rows:

  # perldl with "do_print" set to true
  pdl> sequence(3,2)
  [
   [0 1 2]
   [3 4 5]
  ]
  # in memory, the values are: 0 1 2 3 4 5
  pdl> sequence(3,2)->at(0,1)
  3

This uses "column-major" indexing, though the layout is "row-major".
This is different from the mathematical convention for matrices,
where C<0,1> would be (row,column), and give a value of "1". That
also ignores the zero-based indexing in C-like languages, versus
the 1-based indexing in Fortran and maths.

For interoperability between PDL and Fortran (and also OpenGL), an
ndarray would need to be transposed with the C<transpose> operation.
Alternatively, functions in either of those often have a "transpose"
flag for smooth interoperation.

=head2 Dimension lists

A ndarray (PDL variable), in general, is an N-dimensional array where N can be
...
integer size.  The C<perl> interpreter treats each ndarray as a special type of
...
=item nelem - the total number of elements in an ndarray

=item ndims - returns the number of dimensions in an ndarray

=item dims - returns the dimension list of an ndarray as a Perl list

=item dim - returns the size of a particular dimension of an ndarray

=back

=head2 Indexing and Dataflow

PDL maintains a notion of "dataflow" between an ndarray and indexed subfields of
...
=item copy - forces an explicit copy of an ndarray

=item sever - breaks the dataflow connection between an ndarray and its parents (if any)

=back

=head2 Broadcasting and Dimension Order

Most PDL operations act on the first few dimensions of their ndarray arguments.  For
...
"active" dimension and the later dimensions are "broadcast" dimensions because they are simply
...
document Perl level broadcasting (see L<PDL::Core/broadcast_define>)

=item *

broadcastids (see L</Explicit broadcasting>)

=item *

L<PDL::Slices/slice> is crucial, useful in niches are L<PDL::Slices/lags>
and L<PDL::Slices/splitdim>

=back

=head1 Indexing and broadcasting with PDL
...
=head2 ndarrays can have children

Central to many of the indexing capabilities of PDL are the relation of
"parent" and "child" between ndarrays. Many of the indexing commands
create a new ndarray from an existing ndarray. The new ndarray is the "child"
and the old one is the "parent". The data of the new ndarray is defined by a
...
routines (e.g. C<< ->index(...) >>) result in physical ndarrays.
All routines compiled by PP can accept affine ndarrays, except
those routines that pass pointers to external library functions,
and those will simply be "physicalised" (memory allocated, data copied in).
There will then be dataflow back and forth, in a lazy-evaluation way.
See L<PDL::Dataflow> for more.
...
All three "child" ndarrays are children of C<$im> or in the other (largely
...
parent physical ndarray and vice versa (however, the basic "=" assignment
doesn't, use ".=" to obtain that effect. See below for the reasons).
The virtual child ndarrays are
something like "live links" to the "original" parent ndarray. As
...
where C<ind0> specifies what to do with index No 0 of the ndarray C<$x>,
...
=head3 Generalised diagonals

With L<PDL::Slices/diagonal> you can make I<generalised diagonals>.
The best way to explain it is probably to
give an example, here we make an ndarray that refers to the elements along
the space diagonal of its parent ndarray (a cube):

 $cube = zeroes(5,5,5);
 $sdiag = $cube->diagonal(1..3);
...
of elements in the diagonal, e.g.
 ...
 $vpdl = $rect->slice('2:7,0:1,(4),5:4'); # now 6,2,2,2
 $vpdl = $vpdl->diagonal(1..3); # now 6,2

So the elements of $vpdl will then be related to those of its parent
in way we can express as:

  vpdl(i,j) = rect(i+2,j,4,5-j,j)       0<=i<6, 0<=j<2
...
to change anytime. To obtain the desired behaviour, use
 ...
 $x->slice(':,(1)') .= $x->slice('-1:0,(1)')->copy;

which makes a physical copy of the slice or

 $x->slice(':,(1)') .= $x->slice('-1:0,(1)')->sever;

which returns the same slice but first severs the connection of the slice
to its parent.
...
all elements with index C<(X,x,Y)> (C<< 0<=x<size_of_dummy_dim >>) just map to
the element with index C<(X,Y)> of the parent ndarray (where C<X> and C<Y> refer to
...
you have worked with PDL broadcasting you wouldn't want to live without
them again).

 # centroid
 ($xd,$yd) = $im->dims;
 $xc = sum($im*xvals($xd)->dummy(1,$yd))/sum($im);
...
make a unity matrix:
 ...
 $e->diagonal(0,1) .= 1; # set the elements along the diagonal to 1
 print $e;

Or the other diagonal:

 $e->slice(':-1:0')->diagonal(0,1) .= 2;
 print $e;

(Did you notice how we used the slice function to reverse the sequence
...
orthogonal matrix. Often C<xchg> will be used in the context of broadcasting
...
B<undefined>, and will (as of 2.086) throw an error.
...
It is very memory efficient, with only metadata existing in the child.
It provides a very flexible and powerful way of accessing portions of
ndarray data.

=head2 How to make things physical again

A "virtual affine" (or "vaffine") ndarray will be automatically
"physicalised" (have memory allocated and its data copied in) if that
is requested with the L<PDL::Core/make_physical> method, if it is
L<sever|PDL::Core/sever>ed, or if it is passed to an operation as a
parameter that is marked as C<[phys]>. See L<PDL::PP> for more.
It will still have dataflow except in the C<sever> case.
...
"broadcasting" with ndarrays: within the framework of PDL it could
...
'magically') generated by PDL based on the dimensions of the ndarrays
...
L<PDL::PP> compiled functions) and that the ndarrays are "broadcast" over.
So much about the terminology and now let's try to shed some light on what it
all means.

=head2 Implicit broadcasting - a first example

There are two slightly different variants of broadcasting. We start
with what we call "implicit broadcasting". Let's pick a practical example
that involves looping of a function over many elements of a
ndarray. Suppose we have an RGB image that we want to convert to
grey-scale. The RGB image is represented by a 3-dim ndarray C<im(3,x,y)> where
...
descriptions by L<PDL::PP>, later just called PP-functions) expects a
defined (minimum) number of dimensions (we call them core dimensions)
from each of its ndarray arguments. The L<inner|PDL::Primitive/inner> function
...
PP-functions with ndarrays that have I<more> than the required core
...
(and only) dimension of C<$w>. The second dimension is the first broadcast
dimension (of size C<x>) and the third is here the second broadcast
dimension (of size C<y>). The output ndarray is automatically created (as
...
dimensions of the auto-created ndarray (here C<0D+2D=2D> to yield a 2D
...
output ndarray C<$grey(i,j)> to the result of each computation. We could
...
might be going on:
 ...
 sub lookup {
   my ($im,$palette) = @_;
   $palette->xchg(0,1)->index($im->indx->dummy(0,$palette->dim(0)));
 }
...
the PP-function, see B<Appendix A> for a list of PDL primitives) and
extra dimensions which comprises all remaining dimensions of that
ndarray. For example calling a function C<func> with the signature
C<func((n,m),[o](n))> with an ndarray C<$x(2,4,7,1,3)> as C<f($x,($o = null))>
...
Core, or "active", dimensions are identified with the first N dimensions of the
...
ndarray is "promoted" as if having a dummy dimension of size equal to the
...
size of the extra dimension in an ndarray is 1 it is implicitly treated
as a dummy dimension of size equal to ("inflated" to) that loop dim size when
performing the broadcast loop.

=item R5

If output auto-creation is used (by setting the relevant ndarray to
C<< PDL->null >> before invocation) the number of dimensions of the created
...
and you call it with 3 ndarrays C<$x(5,3,10,11)>,
...
We can call the conversion with an ndarray representing a pixel (C<im(3)>),
...
C<(3,...)> the automatically created output ndarray will contain the right
...
broadcasting>. You can easily convince yourself that calling with a color
...
broadcasting is done by adding appropriate dummy dimensions to C<xvals(...)>
according to R3.
...
it will only "gobble up" the first dimension of a given input ndarray. But
...
operations". These operations take a N-D ndarray as input and return a
(N-1)-D "projected" ndarray. These operations are often performed with
...
the ndarray in question C<clump> comes in handy. As an example look at a
definition of C<sum> (summarised from F<Basic/Ufunc/ufunc.pd>):
...
C<((n),(),[o]())> (see B<Appendix A>) with the core functionality that
C<index(pdl (0,2,4,5),2,($ret=null))> will return the element with index
2 of the first input ndarray. In this case, C<$ret> will contain the value 4.
...
desired size (by rule R4, in our example the size would be 3 assuming
a palette of size C<(3,x)>). Since situations like this do occur often in
practice this is actually why rule R4 has been introduced (the part
that makes dimensions of size 1 fit to the broadcast loop dim size). So
we can just say

 pdl> index($palette->xchg(0,1),$im->indx->dummy(0),($res=null));
...
L<PDL::Slices/whichND> for even more powerful constructs.
...
"empty" ndarray (for those interested in details there is a flag
...
B<explicit broadcasting>, the second variant of broadcasting).
...
limitation on output creation when using B<explicit broadcasting>.

=head2 Explicit broadcasting
...
too surprising that this variant of the game is called I<explicit broadcasting>.
...
L<broadcast|PDL::Core/PDL::broadcast>
and L<unbroadcast|PDL::Slices/unbroadcast>.
We start with an example that illustrates typical
usage of the former:

=for comment TODO: elaborate on why it's the worst or remove the comment
 ...
 pdl> $mat->broadcast(0) += $line

In this example, C<< $mat->broadcast(0) >> tells PDL that you want the second
...
C<broadcast> takes a list of numbers as arguments which explicitly
...
have been specified in a call to C<broadcast> as being taken away from
the set of normal dimensions and put on a separate stack. So assuming
we have an ndarray C<x(4,7,2,8)> saying

 $y = $x->broadcast(2,1)

creates a new virtual ndarray of dimension C<y(4,8)> (which we call the
remaining dims) that also has 2 broadcast dimensions of size C<(2,7)>. For
...
broadcast loop, I<explicit loop dimensions> are broadcasted over first
...
I<implicit loop dimension>. For all ndarrays that have a given
I<broadcast/extra dimension> the size must be equal to the size of the
respective I<explicit/implicit loop dimension> or 1; otherwise you
raise a runtime exception. If the size of a I<broadcast/extra dimension>
...
If an ndarray doesn't have a I<broadcast/extra dimension> that corresponds to
an I<explicit/implicit loop dimension>, in the broadcast loop this
ndarray is treated as if having a dummy dimension of size equal to the
size of that loop dimension.

=item B4a

All ndarrays that do have I<broadcast dimensions> must have the same number of
broadcast dimensions.

=item B5

Output auto-creation cannot be used if any of the ndarray arguments has any
I<broadcast dimensions>. Otherwise R5 applies.

=back

The same restrictions apply with regard to implicit dummy dimensions
(created by application of B4) as already mentioned in the section
...
and you call it with 3 ndarrays C<a(5,3,10,11)>, C<b(3,5,10,1,12)>, C<c(10)> and an
output ndarray C<d(3,11,5,10,12)> (which can here I<not> be automatically
created) as
...
the form C<pdl(core dims){broadcast dims}[extra dims]>):
...
that the number of explicit loop dimensions is 2 (by B1b from C<$a> and C<$b>)
with sizes C<(3,11)> (by B2); 2 implicit loop dimensions (by B1a from C<$b>
 ...
 ** then  A^T * $tmp by broadcasted inner product
 ...
 $tmp->broadcast(0) /= $evals;    #  broadcasted division
...
=head2 Implicit versus explicit broadcasting
...
computing facilities/parallel processing) PDL broadcasting is
